...
def choose_data_type():
    ...
    flag = True
    while flag:
        print('Выбирете какие данные добавить: ', '1 - телефонный номер', '2 - рабочий номер', '3 - email',
              '4 - дата рождения', sep='\n')
        type_data = input('Выбирете цифру (тип телефона): ')
        match type_data:
            case '1':
                type_data = 'мобильный номер'
                flag = False
            case '2':
                type_data = 'рабочий номер'
                flag = False
            case '3':
                type_data = 'email'
                flag = False
            case '4':
                type_data = 'дата рождения'
                flag = False
            case _:
                flag = True
    return type_data
    ...
    if answer != None:
        ...
        while flag:
            ...
            if dict_res.get(type_data, 0) != 0:
                try:
                    position = pb_local[f_name][dict_res[type_data][0]].index(dict_res[type_data][1])
                except:
                    position = -1

                if position >= 0:
                    pb_local[f_name][dict_res[type_data][0]][position] = input('Введите новые данные: ')
                else:
                    pb_local[f_name][dict_res[type_data][0]] = [(input('Введите новые данные: '))]
